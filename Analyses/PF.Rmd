...
# phloem-feeder counts (2022)
pf_counts <- read.csv("data/pf_counts_2019_2022.csv")
...
# phloem-feeder counts (2022)
pf_counts <- pf_counts %>% 
  mutate(across(c(site : branchID), as.factor))
...
```{r pf_counts_by_ants, echo = F}

pf_counts %>% 
  ggplot(aes(x = ant_treatment, y = count_pf)) +
  geom_boxplot() 


pfc_mod <- glm(count_pf ~ ant_treatment + site,
                   contrasts = list(site = "contr.sum"),
                   family = poisson(),
                   data = pf_counts)


kable(tidy(anova(pfc_mod, test = "LRT")))
...
```{r branch_level_mods, echo = F}

branch <- dat1 %>% 
  group_by(branchID, year, site, pf_treatment, ant_treatment, para) %>% 
  tally() %>% 
  mutate(not_para = n - para,
         prop.parasitism = para/n)

branch.mod <- glm(prop.parasitism ~ pf_treatment * ant_treatment +
                    year + site, family = binomial(), data = branch,
                  weights = n)

# overdispersion_test(branch.mod)

anova(branch.mod)

# get bootstrapped models to generate CIs

boots <- bootstraps(branch, times = 2000, apparent = T)

# function to fit glm
fit_glm_bootstrap <- function(split) {
  glm(prop.parasitism ~ pf_treatment * ant_treatment + year + site,
      family = binomial(),
      weights = n,
  analysis(split))
}

# this creates a nested data frame where each value in 'model' is a bootstrap
# iteration of the GLM

boot_models <- # this takes a minute or two.
  boots %>% 
  mutate(model = map(splits, fit_glm_bootstrap),
         coef_info = map(model, tidy))

# unnest the coefficients from each bootstrap
boot_coefs <- 
  boot_models %>% 
  unnest(coef_info)

# calculate percentile based CIs
percentile_intervals <- int_pctl(boot_models, coef_info)

# plot the bootstraps for each variable
...
```{r branch_results, echo = F}

branchCIs <- percentile_intervals %>% 
...
write.csv(branchCIs,
          file = "data/branch_level_results.csv",
          row.names = F, quote = F)

kable(branchCIs, format = "markdown", digits = 2)
...
## remove interaction
branch_tmbmodH1 <- update(branch_tmbmod, ~.-ant_treatment:pf_treatment)
branch_tmbmodH0 <- update(branch_tmbmodH1, ~.-pf_treatment)
anova(branch_tmbmodH1, branch_tmbmodH0)
```
